...
Now start adding your routes as class fields with the desired route names like so:
**You don't need to annotate very single route with @MaterialRoute() or @CupertinoRoute()**

```dart
@autoRouter
class $Router {
 // use @initial to annotate your initial route.
  @initial
  HomeScreen homeScreenRoute; // your desired route name

  SecondScreen secondScreenRoute;

  //only use the @MaterialRoute() or @CupertinoRoute() annotations if you need to customize your route
...
if you want the generator to run one time and exists use
