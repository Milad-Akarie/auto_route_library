...
const _validRouteAnnotations = ['Initial', 'MaterialRoute', 'CupertinoRoute', 'CustomRoute'];
...
class RouterConfigVisitor extends SimpleElementVisitor<RouteConfig> {
  ...
  RouteConfig visitFieldElement(FieldElement field) {
    ...
    routeConfig.hasWrapper =
		    classElement.allSupertypes.map<String>((el) => el.displayName).contains('AutoRouteWrapper');
    final constructor = (type.element as ClassElement).unnamedConstructor;
    if (constructor != null && constructor.parameters.isNotEmpty) {
	    routeConfig.parameters = constructor.parameters.map((p) => RouteParameter.fromParameterElement(p)).toList();
    }
  ...
  void _extractMetaData(FieldElement field, RouteConfig routeConfig) {
    if (field.metadata != null && field.metadata.isNotEmpty) {
      ...
      final type = autoRoute.type.toString();
	    ...
	    routeConfig.fullscreenDialog = autoRoute.getField('fullscreenDialog')?.toBoolValue();
	    routeConfig.maintainState = autoRoute.getField('maintainState')?.toBoolValue();
	    ...
	    if (type == 'CupertinoRoute') {
		    ...
		    routeConfig.cupertinoNavTitle = autoRoute.getField('title')?.toStringValue();
	    } else if (type == 'CustomRoute') {
        routeConfig.routeType = RouteType.custom;
        routeConfig.durationInMilliseconds = autoRoute.getField('durationInMilliseconds').toIntValue();
        routeConfig.customRouteOpaque = autoRoute.getField('opaque')?.toBoolValue();
        routeConfig.customRouteBarrierDismissible = autoRoute.getField('barrierDismissible')?.toBoolValue();
        final function = autoRoute.getField('transitionsBuilder')?.toFunctionValue();
        if (function != null) {
